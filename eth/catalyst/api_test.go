...
func equalBody(a *types.Body, b *engine.ExecutionPayloadBody) bool {
	...
	// XXX: astria-geth's fork no longer populates the transaction and requests fields
	// of the block bodies - at least in the test setup. This might be due to how the
	// miner/worker changed, but is not fully understood right now. In this comparator
	// this leads to issues because while `deposits` ends up being an empty slice, while
	// `b.Deposits` is nil
	return len(deposits) == len(b.Deposits) || reflect.DeepEqual(deposits, b.Deposits)
}
