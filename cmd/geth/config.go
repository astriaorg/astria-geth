...
func makeFullNode(ctx *cli.Context) *node.Node {
	...
	if ctx.IsSet(utils.GRPCEnabledFlag.Name) {
		serviceV2, err := execution.NewExecutionServiceServerV2(eth, ctx.Bool(utils.ExecutionSoftAsFirmFlag.Name), ctx.Uint64(utils.ExecutionServiceSoftAsFirmMaxHeightFlag.Name))
		if err != nil {
