...
func (miner *Miner) fillAstriaTransactions(interrupt *atomic.Int32, env *environment, overrideTransactions types.Transactions) error {
	// Use pre ordered array of txs
	astriaTxs := miner.txpool.AstriaOrdered()
	// use the override transactions instead of astria txs
	if overrideTransactions.Len() > 0 {
...
func (miner *Miner) generateWork(params *generateParams) *newPayloadResult {
	...
	if !params.noTxs {
		...
		err := miner.fillAstriaTransactions(interrupt, work, params.overrideTransactions)
